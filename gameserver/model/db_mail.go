...
func (g *PlayerData) MailReadItem(conf []*constant.Item, allSync *AllPlayerSync) (bool, []*proto.Item) {
	...
	for _, v := range conf {
		...
		case constant.MailEquipment:
			EquipmentUniqueId := g.AddEquipment(v.ItemId)
			allSync.EquipmentList = append(allSync.EquipmentList, v.ItemId, EquipmentUniqueId)
			pileItem = append(pileItem, &Material{
				Tid: v.ItemId,
				Num: v.Num,
			})
			...
			item = g.GetEquipmentItem(EquipmentUniqueId)
